...
struct vex2 {
    T x, y;
    template <typename _T> inline operator vex2<_T>() { return { _T(x),_T(y) }; };
    template <typename _T> inline operator vex3<_T>() { return { _T(x),_T(y),0 }; };
    template <typename _T> inline operator vex4<_T>() { return { _T(x),_T(y),0,0 }; };
...
#undef __VEX2IM_FUNCS__
    ...
    template <typename _T> inline vex2(const _T& v) { x = y = T(v); };
    template <typename _T> inline vex2(const vex2<_T>& v) { x = T(v.x); y = T(v.y); };
    template <typename _T> inline vex2(const vex3<_T>& v) { x = T(v.x); y = T(v.y); };
    template <typename _T> inline vex2(const vex4<_T>& v) { x = T(v.x); y = T(v.y); };
    template <typename _T1, typename _T2> inline vex2(const _T1& v1, const _T2& v2) { x = T(v1); y = T(v2); };

    inline vex2<T>& operator=(const T& v) { x = y = v; return *this; };
    inline vex2<T>& operator=(T* v) { x = v[0]; y = v[1]; return *this; };
    inline vex2<T>& operator=(const vex2<T>& v) { x = v.x; y = v.y; return *this; };
    ...
    inline vex2<T> operator%(const T& v) const { return vex2<T>(std::rand() % int(v), std::rand() % int(v)); };
    inline vex2<T> operator%(T* v) const { return vex2<T>(std::rand() % int(v[0]), std::rand() % int(v[1])); };
    inline vex2<T> operator%(const vex2<T>& v) const { return vex2<T>(std::rand() % int(v.x), std::rand() % int(v.y)); };
    inline vex2<T>& operator%=(const T& v) { x = std::rand() % int(v); y = std::rand() % int(v); return *this; }
    inline vex2<T>& operator%=(T* v) { x = std::rand() % int(v[0]); y = std::rand() % int(v[1]); return *this; };
    inline vex2<T>& operator%=(const vex2<T>& v) { x = std::rand() % int(v.x); y = std::rand() % int(v.y); return *this; };

    inline vex2<T> operator|(const T& v) const { return std::rand() % 2 == 0 ? vex2<T>(v) : *this; };
    inline vex2<T> operator|(const vex2<T>& v) const { return std::rand() % 2 == 0 ? vex2<T>(v) : *this; };
    inline vex2<T>& operator|=(const vex2<T>& v) { *this = std::rand() % 2 == 0 ? vex2<T>(v) : *this; };
    //inline vex2<T> operator|(const vex3<T>& v) const { return std::rand() % 2 == 0 ? vex2<T>(v) : *this; };
    //inline vex2<T>& operator|=(const vex3<T>& v) { *this = std::rand() % 2 == 0 ? vex2<T>(v) : *this; };
    //inline vex2<T> operator|(vex4<T> v) const { return std::rand() % 2 == 0 ? vex2<T>(v) : *this; };
    //inline vex2<T>& operator|=(vex4<T> v) { *this = std::rand() % 2 == 0 ? vex2<T>(v) : *this; };

    inline vex2<T>& operator++() { x++; y++; return *this; };
    inline vex2<T>& operator--() { x--; y--; return *this; };

    inline vex2<T>& operator+=(const T& v) { x += v; y += v; return *this; };
    inline vex2<T>& operator+=(T* v) { x += v[0]; y += v[1]; return *this; };
    inline vex2<T>& operator+=(const vex2<T>& v) { x += v.x; y += v.y; return *this; };
    inline vex2<T>& operator-=(const T& v) { x -= v; y -= v; return *this; };
    inline vex2<T>& operator-=(T* v) { x -= v[0]; y -= v[1]; return *this; };
    ...
    inline vex2<T>& operator/=(T* v) { x /= v[0]; y /= v[1]; return *this; };
    inline vex2<T>& operator/=(const vex2<T>& v) { x /= v.x; y /= v.y; return *this; };
    inline vex2<T> operator+(const T& v) const { return vex2<T>(x + v, y + v); };
    inline vex2<T> operator+(T* v) const { return vex2<T>(x + v[0], y + v[1]); };
    inline vex2<T> operator+(const vex2<T>& v) const { return vex2<T>(x + v.x, y + v.y); };
    inline vex2<T> operator-(const T& v) const { return vex2<T>(x - v, y - v); };
    inline vex2<T> operator-(T* v) const { return vex2<T>(x - v[0], y - v[1]); };
    inline vex2<T> operator-(const vex2<T>& v) const { return vex2<T>(x - v.x, y - v.y); };
    inline vex2<T> operator*(const T& v) const { return vex2<T>(x * v, y * v); };
    ...
    inline vex2<T> operator/(T* v) const { return vex2<T>(x / v[0], y / v[1]); };
    inline vex2<T> operator/(const vex2<T>& v) const { return vex2<T>(x / v.x, y / v.y); };
    inline vex2<T> operator^(const T& v) const { return vex2<T>(std::pow(x, v), std::pow(y, v)); };
    inline vex2<T> operator^(T* v) const { return vex2<T>(std::pow(x, v[0]), std::pow(y, v[1])); };
    inline vex2<T> operator^(const vex2<T>& v) const { return vex2<T>(std::pow(x, v.x), std::pow(y, v.y)); };

    inline vex2<bool> operator==(const T& v)  const { return vex2<bool>(x == v, y == v); };
    inline vex2<bool> operator==(T* v) const { return vex2<bool>(x == v[0], y == v[1]); };
    inline vex2<bool> operator==(const vex2<T>& v) const { return vex2<bool>(x == v.x, y == v.y); };
    inline vex2<bool> operator!=(const T& v)  const { return vex2<bool>(x != v, y != v); };
    inline vex2<bool> operator!=(T* v) const { return vex2<bool>(x != v[0], y != v[1]); };
    inline vex2<bool> operator!=(const vex2<T>& v) const { return vex2<bool>(x != v.x, y != v.y); };
    inline vex2<bool> operator>(const T& v)  const { return vex2<bool>(x > v, y > v); };
    ...
    inline vex2<T>& operator= (vex3<T> v) { x = v.x;   y = v.y;     return *this; };
    inline vex2<T>& operator+=(vex3<T> v) { x += v.x;  y += v.y;    return *this; };
    inline vex2<T>& operator-=(vex3<T> v) { x -= v.x;  y -= v.y;    return *this; };
    inline vex2<T>& operator*=(vex3<T> v) { x *= v.x;  y *= v.y;    return *this; };
    inline vex2<T>& operator/=(vex3<T> v) { x /= v.x;  y /= v.y;    return *this; };
    inline vex2<T> operator+(vex3<T> v) const { return vex2<T>(x + v.x, y + v.y); };
    inline vex2<T> operator-(vex3<T> v) const { return vex2<T>(x - v.x, y - v.y); };
    inline vex2<T> operator*(vex3<T> v) const { return vex2<T>(x * v.x, y * v.y); };
    inline vex2<T> operator/(vex3<T> v) const { return vex2<T>(x / v.x, y / v.y); };
    inline vex2<bool> operator==(vex3<T> v) const { return vex2<bool>(x == v.x, y == v.y); };
    inline vex2<bool> operator!=(vex3<T> v) const { return vex2<bool>(x != v.x, y != v.y); };
    inline vex2<bool> operator>(vex3<T> v)  const { return vex2<bool>(x > v.x, y > v.y); };
    inline vex2<bool> operator<(vex3<T> v)  const { return vex2<bool>(x < v.x, y < v.y); };
    inline vex2<bool> operator>=(vex3<T> v) const { return vex2<bool>(x >= v.x, y >= v.y); };
    ...
    inline vex2<T>& operator= (vex4<T> v) { x = v.x;   y = v.y;     return *this; };
    inline vex2<T>& operator+=(vex4<T> v) { x += v.x;  y += v.y;    return *this; };
    inline vex2<T>& operator-=(vex4<T> v) { x -= v.x;  y -= v.y;    return *this; };
    inline vex2<T>& operator*=(vex4<T> v) { x *= v.x;  y *= v.y;    return *this; };
    inline vex2<T>& operator/=(vex4<T> v) { x /= v.x;  y /= v.y;    return *this; };
    inline vex2<T>& operator+(vex4<T> v)  const { return vex2<T>(x + v.x, y + v.y); };
    inline vex2<T>& operator-(vex4<T> v)  const { return vex2<T>(x - v.x, y - v.y); };
    inline vex2<T>& operator*(vex4<T> v)  const { return vex2<T>(x * v.x, y * v.y); };
    inline vex2<T>& operator/(vex4<T> v)  const { return vex2<T>(x / v.x, y / v.y); };
    inline vex2<bool> operator==(vex4<T> v)  const { return vex2<bool>(x == v.x, y == v.y); };
    inline vex2<bool> operator!=(vex4<T> v)  const { return vex2<bool>(x != v.x, y != v.y); };
    inline vex2<bool> operator>(vex4<T> v)   const { return vex2<bool>(x > v.x, y > v.y); };
    inline vex2<bool> operator<(vex4<T> v)   const { return vex2<bool>(x < v.x, y < v.y); };
    inline vex2<bool> operator>=(vex4<T> v)  const { return vex2<bool>(x >= v.x, y >= v.y); };
...
struct vex3 {
    T x, y, z;
    template <typename _T> inline operator vex2<_T>() { return { _T(x),_T(y) }; };
    template <typename _T> inline operator vex3<_T>() { return { _T(x),_T(y),_T(z) }; };
    template <typename _T> inline operator vex4<_T>() { return { _T(x),_T(y),_T(z),0 }; };
    inline operator T* () { return arr(); };
    inline operator T* () const { return arr(); };
...
#undef __VEX3IM_FUNCS__
    ...
    vex3() = default;
    template <typename _T> inline vex3(const _T& v) { x = y = z = T(v); };
    template <typename _T> inline vex3(_T* v) { x = T(v[0]); y = T(v[1]); z = T(v[2]); };
    template <typename _T> inline vex3(const vex2<_T>& v) { x = T(v.x); y = T(v.y); };
    template <typename _T> inline vex3(const vex3<_T>& v) { x = T(v.x); y = T(v.y); z = T(v.z); };
    template <typename _T> inline vex3(const vex4<_T>& v) { x = T(v.x); y = T(v.y); z = T(v.z); };
    template <typename _T1, typename _T2> inline vex3(const _T1& v1, const _T2& v2) { x = T(v1); y = T(v2); };
    template <typename _T1, typename _T2, typename _T3> inline vex3(const _T1& v1, const _T2& v2, const _T3& v3) { x = T(v1); y = T(v2); z = T(v3); };

    template <typename _T1, typename _T2> inline vex3(const vex2<_T1>& v1_2, const _T2& v3) { x = T(v1_2.x); y = T(v1_2.y); z = T(v3); };
    template <typename _T1, typename _T2> inline vex3(const _T1& v1, const vex2<_T2>& v2_3) { x = T(v1); y = T(v2_3.x); z = T(v2_3.y); };
    
    inline vex3<T>& operator=(const T& v) { x = y = z = v; return *this; };
    inline vex3<T>& operator=(T* v) { x = v[0]; y = v[1]; z = v[2]; return *this; };
    inline vex3<T>& operator=(const vex3<T>& v) { x = v.x; y = v.y; z = v.z; return *this; };
    ...
    inline vex3<T> operator%(const T& v) const { return vex3<T>(std::rand() % int(v), std::rand() % int(v), std::rand() % int(v)); };
    inline vex3<T> operator%(T* v) const { return vex3<T>(std::rand() % int(v[1]), std::rand() % int(v[2]), std::rand() % int(v[3])); };
    inline vex3<T> operator%(const vex3<T>& v) const { return vex3<T>(std::rand() % int(v.x), std::rand() % int(v.y), std::rand() % int(v.z)); };
    inline vex3<T>& operator%=(const T& v) { x = std::rand() % int(v); y = std::rand() % int(v); z = std::rand() % int(v); return *this; };
    inline vex3<T>& operator%=(T* v) { x = std::rand() % int(v[0]); y = std::rand() % int(v[1]); z = std::rand() % int(v[2]); return *this; };
    inline vex3<T>& operator%=(const vex3<T>& v) { x = std::rand() % int(v.x); y = std::rand() % int(v.y); z = std::rand() % int(v.z); return *this; };

    inline vex3<T> operator|(const T& v) const { return std::rand() % 2 == 0 ? vex3<T>(v) : *this; };
    inline vex3<T> operator|(const vex3<T>& v) const { return std::rand() % 2 == 0 ? vex3<T>(v) : *this; };
    inline vex3<T>& operator|=(const vex3<T>& v) { *this = std::rand() % 2 == 0 ? vex3<T>(v) : *this; };
    //inline vex3<T> operator|(vex2<const T&> v) const { return std::rand() % 2 == 0 ? vex3<T>(v) : *this; };
    //inline vex3<T>& operator|=(vex2<const T&> v) { *this = std::rand() % 2 == 0 ? vex3<T>(v) : *this; };
    //inline vex3<T> operator|(vex4<const T&> v) const { return std::rand() % 2 == 0 ? vex3<T>(v) : *this; };
    //inline vex3<T>& operator|=(vex4<const T&> v) { *this = std::rand() % 2 == 0 ? vex3<T>(v) : *this; };

    inline vex3<T>& operator++() { x++; y++; z++; return *this; };
    inline vex3<T>& operator--() { x--; y--; z--; return *this; };

    inline vex3<T>& operator+=(const T& v) { x += v; y += v; z += v; return *this; };
    inline vex3<T>& operator+=(T* v) { x += v[0]; y += v[1]; z += v[2]; return *this; };
    inline vex3<T>& operator+=(const vex3<T>& v) { x += v.x; y += v.y; z += v.z; return *this; };
    inline vex3<T>& operator-=(const T& v) { x -= v; y -= v; z -= v; return *this; };
    inline vex3<T>& operator-=(T* v) { x -= v[0]; y -= v[1]; z -= v[2]; return *this; };
    ...
    inline vex3<T>& operator/=(T* v) { x /= v[0]; y /= v[1]; z /= v[2]; return *this; };
    inline vex3<T>& operator/=(const vex3<T>& v) { x /= v.x; y /= v.y; z /= v.z; return *this; };
    inline vex3<T> operator+(const T& v) const { return vex3<T>(x + v, y + v, z + v); };
    inline vex3<T> operator+(T* v) const { return vex3<T>(x + v[0], y + v[1], z + v[2]); };
    inline vex3<T> operator+(const vex3<T>& v) const { return vex3<T>(x + v.x, y + v.y, z + v.z); };
    inline vex3<T> operator-(const T& v) const { return vex3<T>(x - v, y - v, z - v); };
    inline vex3<T> operator-(T* v) const { return vex3<T>(x - v[0], y - v[1], z - v[2]); };
    inline vex3<T> operator-(const vex3<T>& v) const { return vex3<T>(x - v.x, y - v.y, z - v.z); };
    inline vex3<T> operator*(const T& v) const { return vex3<T>(x * v, y * v, z * v); };
    ...
    inline vex3<T> operator/(T* v) const { return vex3<T>(x / v[0], y / v[1], z / v[2]); };
    inline vex3<T> operator/(const vex3<T>& v) const { return vex3<T>(x / v.x, y / v.y, z / v.z); };
    inline vex3<T> operator^(const T& v) const { return vex3<T>(std::pow(x, v), std::pow(y, v), std::pow(z, v)); };
    inline vex3<T> operator^(T* v) const { return vex3<T>(std::pow(x, v[0]), std::pow(y, v[1]), std::pow(z, v[2])); };
    inline vex3<T> operator^(const vex3<T>& v) const { return vex3<T>(std::pow(x, v.x), std::pow(y, v.y), std::pow(z, v.z)); };

    inline vex3<bool> operator==(const T& v)  const { return vex3<bool>(x == v, y == v, z == v); };
    inline vex3<bool> operator==(T* v) const { return vex3<bool>(x == v[0], y == v[1], z == v[2]); };
    inline vex3<bool> operator==(const vex3<T>& v) const { return vex3<bool>(x == v.x, y == v.y, z == v.z); };
    inline vex3<bool> operator!=(const T& v)  const { return vex3<bool>(x != v, y != v, z != v); };
    inline vex3<bool> operator!=(T* v) const { return vex3<bool>(x != v[0], y != v[1], z != v[2]); };
    inline vex3<bool> operator!=(const vex3<T>& v) const { return vex3<bool>(x != v.x, y != v.y, z != v.z); };
    inline vex3<bool> operator>(const T& v)  const { return vex3<bool>(x > v, y > v, z > v); };
    ...
    inline vex3<T>& operator=(const vex2<T>& v)  { x =  v.x;   y = v.y;    return *this; };
    inline vex3<T>& operator+=(const vex2<T>& v) { x += v.x;  y += v.y;    return *this; };
    inline vex3<T>& operator-=(const vex2<T>& v) { x -= v.x;  y -= v.y;    return *this; };
    inline vex3<T>& operator*=(const vex2<T>& v) { x *= v.x;  y *= v.y;    return *this; };
    inline vex3<T>& operator/=(const vex2<T>& v) { x /= v.x;  y /= v.y;    return *this; };
    inline vex3<T> operator+(const vex2<T>& v) const { return vex3<T>(x + v.x, y + v.y); };
    inline vex3<T> operator-(const vex2<T>& v) const { return vex3<T>(x - v.x, y - v.y); };
    inline vex3<T> operator*(const vex2<T>& v) const { return vex3<T>(x * v.x, y * v.y); };
    inline vex3<T> operator/(const vex2<T>& v) const { return vex3<T>(x / v.x, y / v.y); };
    inline vex2<bool> operator==(const vex2<T>& v) const { return vex2<bool>(x == v.x, y == v.y); };
    ...
    inline vex3<T>& operator=(vex4<T> v)  { x =  v.x;  y =  v.y;  z =  v.z;  return *this; };
    inline vex3<T>& operator+=(vex4<T> v) { x += v.x;  y += v.y;  z += v.z;  return *this; };
    inline vex3<T>& operator-=(vex4<T> v) { x -= v.x;  y -= v.y;  z -= v.z;  return *this; };
    inline vex3<T>& operator*=(vex4<T> v) { x *= v.x;  y *= v.y;  z *= v.z;  return *this; };
    inline vex3<T>& operator/=(vex4<T> v) { x /= v.x;  y /= v.y;  z /= v.z;  return *this; };
    inline vex3<T>& operator+(vex4<T> v) const { return vex3<T>(x + v.x, y + v.y, z + v.z); };
    inline vex3<T>& operator-(vex4<T> v) const { return vex3<T>(x - v.x, y - v.y, z - v.z); };
    inline vex3<T>& operator*(vex4<T> v) const { return vex3<T>(x * v.x, y * v.y, z * v.z); };
    inline vex3<T>& operator/(vex4<T> v) const { return vex3<T>(x / v.x, y / v.y, z / v.z); };
    inline vex3<bool> operator==(vex4<T> v) const { return vex3<bool>(x == v.x, y == v.y, z == v.z); };
    inline vex3<bool> operator!=(vex4<T> v) const { return vex3<bool>(x != v.x, y != v.y, z != v.z); };
    inline vex3<bool> operator>(vex4<T> v)  const { return vex3<bool>(x > v.x, y > v.y, z > v.z); };
    inline vex3<bool> operator<(vex4<T> v)  const { return vex3<bool>(x < v.x, y < v.y, z < v.z); };
    inline vex3<bool> operator>=(vex4<T> v) const { return vex3<bool>(x >= v.x, y >= v.y, z >= v.z); };
...
struct vex4 {
    ...
    template <typename _T> inline operator vex2<_T>() { return { _T(x),_T(y) }; };
    template <typename _T> inline operator vex3<_T>() { return { _T(x),_T(y),_T(z) }; };
    template <typename _T> inline operator vex4<_T>() { return { _T(x),_T(y),_T(z),_T(w) }; };
    inline operator T* () { return arr(); };
    inline operator T* () const { return arr(); };
...
#undef __VEX4IM_FUNCS__
    ...
    vex4() = default;
    template <typename _T> inline vex4(const _T& v) { x = y = z = w = T(v); };
    template <typename _T> inline vex4(_T* v) { x = T(v[0]); y = T(v[1]); z = T(v[2]); w = T(v[3]); };
    template <typename _T> inline vex4(const vex2<_T>& v) { x = T(v.x); y = T(v.y); };
    template <typename _T> inline vex4(const vex3<_T>& v) { x = T(v.x); y = T(v.y); z = T(v.z); };
    template <typename _T> inline vex4(const vex4<_T>& v) { x = T(v.x); y = T(v.y); z = T(v.z); w = T(v.w); };
    template <typename _T1, typename _T2> inline vex4(const _T1& v1, const _T2& v2) { x = T(v1); y = T(v2); };
    template <typename _T1, typename _T2, typename _T3> inline vex4(const _T1& v1, const _T2& v2, const _T3& v3) { x = T(v1); y = T(v2); z = T(v3); };
    template <typename _T1, typename _T2, typename _T3, typename _T4> inline vex4(const _T1& v1, const _T2& v2, const _T3& v3, const _T4& v4) { x = T(v1); y = T(v2); z = T(v3); w = T(v4); };

    template <typename _T1, typename _T2> inline vex4(const _T1& v1, const vex3<_T2>& v2_3_4) { x = T(v1); y = T(v2_3_4.x); z = T(v2_3_4.y); w = T(v2_3_4.z); };
    template <typename _T1, typename _T2> inline vex4(const vex3<_T1>& v1_2_3, const _T2& v4) { x = T(v1_2_3.x); y = T(v1_2_3.y); z = T(v1_2_3.z); w = T(v4); };

    template <typename _T1, typename _T2> inline vex4(const vex2<_T1>& v1_2, _T2& v3) { x = T(v1_2.x); y = T(v1_2.y); z = T(v3); };
    template <typename _T1, typename _T2> inline vex4(const _T1& v1, const vex2<_T2>& v2_3) { x = T(v1); y = T(v2_3.x); z = T(v2_3.y); };
    template <typename _T1, typename _T2> inline vex4(const vex2<_T1>& v1_2, const vex2<_T2>& v3_4) { x = T(v1_2.x); y = T(v1_2.y); z = T(v3_4.x); w = T(v3_4.y); };
    template <typename _T1, typename _T2, typename _T3> inline vex4(const vex2<_T1>& v1_2, const _T2& v3, const _T3& v4) { x = T(v1_2.x); y = T(v1_2.y); z = T(v3); w = T(v4); };
    template <typename _T1, typename _T2, typename _T3> inline vex4(const _T1& v1, const vex2<_T2>& v2_3, const _T3& v4) { x = T(v1); y = T(v2_3.x); z = T(v2_3.y); w = T(v4); };
    template <typename _T1, typename _T2, typename _T3> inline vex4(const _T1& v1, const _T2& v2, const vex2<_T3>& v3_4) { x = T(v1); y = T(v2); z = T(v3_4.x); w = T(v3_4.y); };

    inline vex4<T>& operator=(const T& v) { x = y = z = w = v; return *this; };
    inline vex4<T>& operator=(T* v) { x = v[0]; y = v[1]; z = v[2]; w = v[3]; return *this; };
    inline vex4<T>& operator=(vex4<T> v) { x = v.x; y = v.y; z = v.z; w = v.w; return *this; };
    ...
    inline vex4<T>& operator()(bool b, vex4<T> make) {
        if (b) *this = make;
        return *this;
    };
    inline vex4<T>& operator()(vex2<bool> b, vex4<T> make, const bool& _elif = false) {
        if (_elif) {
            if (b.x) x = make.x;
            else if (b.y) y = make.y;
        }
    ...
    inline vex4<T>& operator()(vex3<bool> b, vex4<T> make, const bool& _elif = false) {
        if (_elif) {
            if (b.x) x = make.x;
            else if (b.y) y = make.y;
            else if (b.z) z = make.z;
    ...
    inline vex4<T>& operator()(vex4<bool> b, vex4<T> make, const bool& _elif = false) {
        if (_elif) {
            if (b.x) x = make.x;
            else if (b.y) y = make.y;
            else if (b.z) z = make.z;
    ...
    inline vex4<T> operator%(const T& v) const { return vex4<T>(std::rand() % int(v), std::rand() % int(v), std::rand() % int(v), std::rand() % int(v)); };
    inline vex4<T> operator%(T* v) const { return vex4<T>(std::rand() % int(v[0]), std::rand() % int(v[1]), std::rand() % int(v[2]), std::rand() % int(v[3])); };
    inline vex4<T> operator%(vex4<T> v) const { return vex4<T>(std::rand() % int(v.x), std::rand() % int(v.y), std::rand() % int(v.z), std::rand() % int(v.w)); };
    inline vex4<T>& operator%=(const T& v) { x = std::rand() % int(v); y = std::rand() % int(v); z = std::rand() % int(v); w = std::rand() % int(v); return *this; };
    inline vex4<T>& operator%=(T* v) { x = std::rand() % int(v[0]); y = std::rand() % int(v[1]); z = std::rand() % int(v[2]); w = std::rand() % int(v[3]); return *this; };
    inline vex4<T>& operator%=(vex4<T> v) { x = std::rand() % int(v.x); y = std::rand() % int(v.y); z = std::rand() % int(v.z); w = std::rand() % int(v.w); return *this; };

    inline vex4<T> operator|(const T& v) const { return std::rand() % 2 == 0 ? vex4<T>(v) : *this; };
    inline vex4<T> operator|(vex4<T> v) const { return std::rand() % 2 == 0 ? vex4<T>(v) : *this; };
    inline vex4<T>& operator|=(vex4<T> v) { *this = std::rand() % 2 == 0 ? vex4<T>(v) : *this; };
    //inline vex4<T> operator|(vex3<const T&> v) const { return std::rand() % 2 == 0 ? vex4<T>(v) : *this; };
    //inline vex4<T>& operator|=(vex3<const T&> v) { *this = std::rand() % 2 == 0 ? vex4<T>(v) : *this; };
    //inline vex4<T> operator|(vex2<const T&> v) const { return std::rand() % 2 == 0 ? vex4<T>(v) : *this; };
    //inline vex4<T>& operator|=(vex2<const T&> v) { *this = std::rand() % 2 == 0 ? vex4<T>(v) : *this; };

    inline vex4<T>& operator++() { x++; y++; z++; w++; return *this; };
    inline vex4<T>& operator--() { x--; y--; z--; w--; return *this; };

    inline vex4<T>& operator+=(const T& v) { x += v; y += v; z += v; w += v; return *this; };
    inline vex4<T>& operator+=(T* v) { x += v[0]; y += v[1]; z += v[2]; w += v[3]; return *this; };
    inline vex4<T>& operator+=(vex4<T> v) { x += v.x; y += v.y; z += v.z; w += v.w; return *this; };
    inline vex4<T>& operator-=(const T& v) { x -= v; y -= v; z -= v; w -= v; return *this; };
    inline vex4<T>& operator-=(T* v) { x -= v[0]; y -= v[1]; z -= v[2]; w -= v[3]; return *this; };
    ...
    inline vex4<T>& operator/=(T* v) { x /= v[0]; y /= v[1]; z /= v[2]; w /= v[3]; return *this; };
    inline vex4<T>& operator/=(vex4<T> v) { x /= v.x; y /= v.y; z /= v.z; w /= v.w; return *this; };
    inline vex4<T> operator+(const T& v) const { return vex4<T>(x + v, y + v, z + v, w + v); };
    inline vex4<T> operator+(T* v) const { return vex4<T>(x + v[0], y + v[1], z + v[2], w + v[3]); };
    inline vex4<T> operator+(vex4<T> v) const { return vex4<T>(x + v.x, y + v.y, z + v.z, w + v.w); };
    inline vex4<T> operator-(const T& v) const { return vex4<T>(x - v, y - v, z - v, w - v); };
    inline vex4<T> operator-(T* v) const { return vex4<T>(x - v[0], y - v[1], z - v[2], w - v[3]); };
    inline vex4<T> operator-(vex4<T> v) const { return vex4<T>(x - v.x, y - v.y, z - v.z, w - v.w); };
    inline vex4<T> operator*(const T& v) const { return vex4<T>(x * v, y * v, z * v, w * v); };
    ...
    inline vex4<T> operator/(T* v) const { return vex4<T>(x / v[0], y / v[1], z / v[2], w / v[3]); };
    inline vex4<T> operator/(vex4<T> v) const { return vex4<T>(x / v.x, y / v.y, z / v.z, w / v.w); };
    inline vex4<T> operator^(const T& v) const { return vex4<T>(std::pow(x, v), std::pow(y, v), std::pow(z, v), std::pow(w, v)); };
    inline vex4<T> operator^(T* v) const { return vex4<T>(std::pow(x, v[0]), std::pow(y, v[1]), std::pow(z, v[2]), std::pow(w, v[3])); };
    inline vex4<T> operator^(vex4<T> v) const { return vex4<T>(std::pow(x, v.x), std::pow(y, v.y), std::pow(z, v.z), std::pow(w, w)); };

    inline vex4<bool> operator==(const T& v)  const { return vex4<bool>(x == v, y == v, z == v, w == v); };
    inline vex4<bool> operator==(T* v) const { return vex4<bool>(x == v[0], y == v[1], z == v[2], w == v[3]); };
    inline vex4<bool> operator==(vex4<T> v) const { return vex4<bool>(x == v.x, y == v.y, z == v.z, w == v.w); };
    inline vex4<bool> operator!=(const T& v)  const { return vex4<bool>(x != v, y != v, z != v, w != v); };
    inline vex4<bool> operator!=(T* v) const { return vex4<bool>(x != v[0], y != v[1], z != v[2], w != v[3]); };
    inline vex4<bool> operator!=(vex4<T> v) const { return vex4<bool>(x != v.x, y != v.y, z != v.z, w != v.w); };
    inline vex4<bool> operator>(const T& v)  const { return vex4<bool>(x > v, y > v, z > v, w > v); };
    ...
    template <typename _T> inline vex4<T> operator<<(_T v) const { vex4<T> t; t.x = v.x; t.y = v.y; t.z = v.z; t.w = v.w; return *this; };
    ...
    template <typename _T> inline vex4<T> operator*(vex2<_T> v) const { return vex4<T>(x * T(v.x), y * T(v.y)); };
    template <typename _T> inline vex4<T> operator/(vex2<_T> v) const { return vex4<T>(x / T(v.x), y / T(v.y)); };
    template <typename _T> inline vex2<bool> operator==(vex2<_T> v) const { return vex2<bool>(x == T(v.x), y == T(v.y)); };
    template <typename _T> inline vex2<bool> operator!=(vex2<_T> v) const { return vex2<bool>(x != T(v.x), y != T(v.y)); };
    template <typename _T> inline vex2<bool> operator>(vex2<_T> v)  const { return vex2<bool>(x > T(v.x), y > T(v.y)); };
    template <typename _T> inline vex2<bool> operator<(vex2<_T> v)  const { return vex2<bool>(x < T(v.x), y < T(v.y)); };
    template <typename _T> inline vex2<bool> operator>=(vex2<_T> v) const { return vex2<bool>(x >= T(v.x), y >= T(v.y)); };
    ...
    template <typename _T> inline vex4<T>& operator*(vex3<_T> v) const { return vex4<T>(x * T(v.x), y * T(v.y), z * T(v.z)); };
    template <typename _T> inline vex4<T>& operator/(vex3<_T> v) const { return vex4<T>(x / T(v.x), y / T(v.y), z / T(v.z)); };
    template <typename _T> inline vex3<bool> operator==(vex3<_T> v) const { return vex3<bool>(x == T(v.x), y == T(v.y), z == T(v.z)); };
    template <typename _T> inline vex3<bool> operator!=(vex3<_T> v) const { return vex3<bool>(x != T(v.x), y != T(v.y), z != T(v.z)); };
    template <typename _T> inline vex3<bool> operator>(vex3<_T> v)  const { return vex3<bool>(x > T(v.x), y > T(v.y), z > T(v.z)); };
    template <typename _T> inline vex3<bool> operator<(vex3<_T> v)  const { return vex3<bool>(x < T(v.x), y < T(v.y), z < T(v.z)); };
    template <typename _T> inline vex3<bool> operator>=(vex3<_T> v) const { return vex3<bool>(x >= T(v.x), y >= T(v.y), z >= T(v.z)); };
    ...
    friend std::ostream& operator<<(std::ostream& os, vex4<T> v) { os << v.x << ' ' << v.y << ' ' << v.z << ' ' << v.w; return os; };
...
template<> _type _type::operator%(_T v) const = delete;     \
template<> _type _type::operator%(_T* v) const = delete;    \
template<> _type& _type::operator%=(_T v) = delete;         \
template<> _type& _type::operator%=(_T* v) = delete;        \
template<> _type _type::operator%(_t<_T> v) const = delete; \
template<> _type& _type::operator%=(_t<_T> v) = delete;     \
template<> _t<bool> _type::operator<(_T v) const = delete;      \
template<> _t<bool> _type::operator<(_T* v) const = delete;     \
template<> _t<bool> _type::operator<(_t<_T> v) const = delete;  \
template<> _t<bool> _type::operator>(_T v) const = delete;      \
template<> _t<bool> _type::operator>(_T* v) const = delete;     \
template<> _t<bool> _type::operator>(_t<_T> v) const = delete;  \
template<> _t<bool> _type::operator<=(_T v) const = delete;     \
...
namespace std {
    ...
    inline vex3d ceil(vex3d v) { return vex3d(std::ceil(v.x), std::ceil(v.y), std::ceil(v.z)); };
    ...
    inline vex4d ceil(vex4d v) { return vex4d(std::ceil(v.x), std::ceil(v.y), std::ceil(v.z), std::ceil(v.w)); };
...
template <typename T> std::vector<T>& operator+=(std::vector<T>& v, vex4<T> i) { v.emplace_back({ i.x, i.y, i.z, i.w }); return v; };
template <typename T> std::vector<T>& operator<<(std::vector<T>& v, vex4<T> i) { v.emplace_back({ i.x, i.y, i.z, i.w }); return v; };
template <typename T> std::vector<T> operator+(std::vector<T> v, vex4<T> i) { std::vector<T> t = v; t.emplace_back({ i.x, i.y, i.z, i.w }); return t; };
...
template <typename T> inline vex4<T> operator+(const char* l, vex4<T> r) { return std::stov4<T>(l) + r; };
template <typename T> inline vex4<T> operator-(const char* l, vex4<T> r) { return std::stov4<T>(l) - r; };
template <typename T> inline vex4<T> operator/(const char* l, vex4<T> r) { return std::stov4<T>(l) / r; };
template <typename T> inline vex4<T> operator*(const char* l, vex4<T> r) { return std::stov4<T>(l) * r; };
...
template <typename T> vex4b limit(vex4<T>& v, vex4<T> min, vex4<T> max, const bool& _restart = false) {
    vex4b b = false;
    if (v.x < min.x) {
        v.x = _restart ? max.x : min.x;
        b.x = true;
...
template <typename T> inline vex4<T> mix(vex4<T> a, vex4<T> b) { return (a + b) / 2; };
...
template <typename T> inline const T& max(vex4<T> v) { return (v.x > v.y && v.x > v.z && v.x > v.w) ? v.x : (v.y > v.z && v.y > v.w) ? v.y : (v.z > v.w) ? v.z : v.w; };
...
template <typename T> inline const T& min(vex4<T> v) { return (v.x < v.y&& v.x < v.z&& v.x < v.w) ? v.x : (v.y < v.z && v.y < v.w) ? v.y : (v.z < v.w) ? v.z : v.w; };
#endif
#endif
